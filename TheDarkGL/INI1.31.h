...
namespace TDEngine
{
	...
	public:
	  ...
	  template<class T>
		static void GetLine(std::stringstream& out, T value)
	  {
		out << value;
	  }

	  static void GetLine(std::stringstream& out, std::string value)
	  {
	...
	public:
	   ...
	   template<class W>
		  std::string get(const W key, const char* def = "")  //Handle C string default value without casting
		  ...
		  if (line[0])
		  {
			 if (!((strlen(line) >= 2) && line[0] == '/' && line[1] == '/'))  //Ignore comment
			 {
				if (line[0] == '[')  //Section
				{
				   ...
				   strncpy(ssection, line + 1, length);  //Count after first bracke
				   current = new keys_t;
				...
				else  //Key
				{
				   ...
				   Converters::GetLine(out2, svalue);
